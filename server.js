...
const pool = usePg ? new Pool({ connectionString: DATABASE_URL, ssl: process.env.NODE_ENV === 'production' ? { rejectUnauthorized:false } : false }) : null;
...
async function initDb(){
 if(!usePg)return;
 await pool.query(`CREATE TABLE IF NOT EXISTS users(id uuid PRIMARY KEY,name text NOT NULL,phone_digits char(4) NOT NULL UNIQUE,created_at timestamptz NOT NULL DEFAULT now());
...
async function findUserById(id){ if(usePg){const r=await pool.query('SELECT id,name,phone_digits as "phoneDigits",created_at as "createdAt" FROM users WHERE id=$1',[id]);return r.rows[0]||null;} return memory.users.get(id)||null; }
async function findUserByDigits(d){ if(usePg){const r=await pool.query('SELECT id,name,phone_digits as "phoneDigits",created_at as "createdAt" FROM users WHERE phone_digits=$1',[d]);return r.rows[0]||null;} return [...memory.users.values()].find(u=>u.phoneDigits===d)||null; }
async function createUser(user){ if(usePg){const r=await pool.query('INSERT INTO users(id,name,phone_digits) VALUES($1,$2,$3) RETURNING id,name,phone_digits as "phoneDigits",created_at as "createdAt"',[user.id,user.name,user.phoneDigits]);return r.rows[0];} memory.users.set(user.id,user);return user; }
async function findAccountByCode(code){ if(usePg){const r=await pool.query('SELECT id,name,invite_code as "inviteCode" FROM accounts WHERE invite_code=$1',[code.toUpperCase()]);return r.rows[0]||null;} return [...memory.accounts.values()].find(a=>a.inviteCode===code.toUpperCase())||null; }
async function accountForUser(userId){ if(usePg){const r=await pool.query('SELECT a.id,a.name,a.invite_code as "inviteCode" FROM accounts a JOIN memberships m ON m.account_id=a.id WHERE m.user_id=$1 ORDER BY a.created_at LIMIT 1',[userId]);return r.rows[0]||null;} const m=[...memory.memberships.values()].find(x=>x.userId===userId);return m?memory.accounts.get(m.accountId):null; }
async function createAccountFor(user,name){let code=accountCode();if(usePg){for(let i=0;i<5;i++){try{const r=await pool.query('INSERT INTO accounts(id,name,invite_code) VALUES($1,$2,$3) RETURNING id,name,invite_code as "inviteCode"',[uid(),`Conta de ${shortName(name)}`,code]);const a=r.rows[0];await pool.query('INSERT INTO memberships(id,user_id,account_id) VALUES($1,$2,$3)',[uid(),user.id,a.id]);return a}catch(e){code=accountCode()}}throw new Error('Não foi possível criar a conta.')}const a={id:uid(),name:`Conta de ${shortName(name)}`,inviteCode:code};memory.accounts.set(a.id,a);memory.memberships.set(uid(),{userId:user.id,accountId:a.id});return a;}
async function joinAccount(user,account){if(usePg){await pool.query('INSERT INTO memberships(id,user_id,account_id) VALUES($1,$2,$3) ON CONFLICT DO NOTHING',[uid(),user.id,account.id]);}else{memory.memberships.set(uid(),{userId:user.id,accountId:account.id});}return account;}
async function memberRows(accountId){if(usePg){const r=await pool.query('SELECT u.id,u.name,u.id as "userId",u.created_at as "createdAt" FROM users u JOIN memberships m ON m.user_id=u.id WHERE m.account_id=$1 ORDER BY m.created_at',[accountId]);return r.rows.map(x=>({...x,short:shortName(x.name),initial:initial(x.name)}));}return [...memory.memberships.values()].filter(m=>m.accountId===accountId).map(m=>{const u=memory.users.get(m.userId);return {...u,userId:u.id,short:shortName(u.name),initial:initial(u.name)}});}
async function recordRows(accountId){if(usePg){const r=await pool.query('SELECT id,"user_id" as "userId",type,amount::float as amount,bank,description,proof_name as "proofName",created_at as "createdAt" FROM records WHERE account_id=$1 ORDER BY created_at DESC',[accountId]);return r.rows;}return [...memory.records.values()].filter(r=>r.accountId===accountId).sort((a,b)=>new Date(b.createdAt)-new Date(a.createdAt)).map(({accountId,...r})=>r);}
async function membership(accountId,userId){if(usePg){const r=await pool.query('SELECT 1 FROM memberships WHERE account_id=$1 AND user_id=$2',[accountId,userId]);return r.rowCount>0;}return [...memory.memberships.values()].some(m=>m.accountId===accountId&&m.userId===userId);}
...
server.listen(PORT,'0.0.0.0',async()=>{
  ...
  try{await initDb(); console.log('Banco de dados inicializado.')}
  catch(err){console.error('Aviso: banco de dados ainda não disponível:',err.message);}
});
